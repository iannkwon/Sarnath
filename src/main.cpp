...
#if defined(NDEBUG)
# error "Sarnath cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4e4bfe92fcdd5ad35a35eb8dff847f8366cc082c463247783dbc926f3f2485f6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sarnath: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sarnath Signed Message:\n";
...
{
    // Sarnath: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Sarnath: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Sarnath: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Sarnath: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sarnath: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sarnath: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4e4bfe92fcdd5ad35a35eb8dff847f8366cc082c463247783dbc926f3f2485f6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Buddha said for the first time to enlighten people";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0430896f1e0b63f5fa01ed26e207efc1fbb845017e5dc012d116db8f60391284a0d0ada558254adbcf5a96942f2c66a8d34287a6b33a9c4acf76f1789a5c05b07e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1527125937; 
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085475237;

        if (fTestNet)
        {
            block.nTime    = 1527128947;
        ...
        assert(block.hashMerkleRoot == uint256("0x351e14dac741a19a91fa7dc36ac59971504b30ee543b15791f3e6e29fc0059f0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb8, 0xdd }; // Sarnath: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SarnathMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SarnathMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SarnathMiner(CWallet *pwallet)
{
    printf("SarnathMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sarnath-miner");
    ...
    try { loop {
        ...
        printf("Running SarnathMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SarnathMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SarnathMiner, pwallet));
}
